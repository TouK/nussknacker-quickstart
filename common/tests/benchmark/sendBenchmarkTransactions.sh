...
cat generateBenchmarkTransactions.sh | runInKafka "tee /tmp/bench.sh"
runInKafka "chmod +x /tmp/bench.sh"
...
runInKafka /tmp/bench.sh $TRANSACTION_COUNT $CLIENT_COUNT $TIME_SPREAD_MINUTES | runInKafka kafka-console-producer --topic $TOPIC --bootstrap-server localhost:9092
