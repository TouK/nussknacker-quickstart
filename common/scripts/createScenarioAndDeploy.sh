...
deploy() {
  ...
  kubectl exec $(kubectl get pods -l app.kubernetes.io/name=apicurio-registry --no-headers -o custom-columns=":metadata.name") -- curl -v http://$NU_QUICKSTART_APICURIO_REGISTRY_SERVICE_HOST:$NU_QUICKSTART_APICURIO_REGISTRY_SERVICE_PORT/apis/ccompat/v6/subjects
  sleep 30
